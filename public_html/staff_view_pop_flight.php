...
<div class="container mt-4 mb-4">
    <div class="row">
        ...
        <div class="col-md-12 well trbox">
            <h2 style="margin: 20px;">Flight Pop Report</h2>
            <?php
                $sql = "SELECT `flightschedule`.`ArrAirport`, count(*) as airport_count FROM `flightorder` INNER JOIN flightschedule ON `flightorder`.`flight_no` = flightschedule.flight_no GROUP BY `flightschedule`.`ArrAirport`";
                mysqli_set_charset($conn, "utf8");
                $rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                while($rc = mysqli_fetch_assoc($rs)) {
                    $chart_stat[$rc["ArrAirport"]] = $rc["airport_count"];
                }

                $sql = "SELECT MONTH(`flightorder`.`DepDateTime`) as Month, count(*) as airport_count FROM `flightorder` INNER JOIN flightschedule ON `flightorder`.`flight_no` = flightschedule.flight_no GROUP BY MONTH(`flightorder`.`DepDateTime`)";
                mysqli_set_charset($conn, "utf8");
                $rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                while($rc = mysqli_fetch_assoc($rs)) {
                    $chart_stat2[$rc["Month"]] = $rc["airport_count"];
                }

                
                $sql = "SELECT WEEKDAY(`flightorder`.`OrderDate`) as weekday, count(*) as weekday_count FROM `flightorder` INNER JOIN flightschedule ON `flightorder`.`flight_no` = flightschedule.flight_no GROUP BY WEEKDAY(`flightorder`.`OrderDate`)";	
                mysqli_set_charset($conn, "utf8");
                $rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                while($rc = mysqli_fetch_assoc($rs)) {
